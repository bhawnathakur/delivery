...
import { CheckeddialogComponent } from './checkeddialog/checkeddialog.component';
...
validate(element, event:MatCheckboxChange ){
 console.log(element.id) ;
  const dialogRef = this.dialog.open(CheckeddialogComponent, {
    width: '300px',
    // data: {name: "this.namel"}
  });
  ...
  dialogRef.afterClosed().subscribe(result => {
    console.log("The dialog was closed" + result);
    if (result == "yes") {
    this.service.checkedOrder(element.id).subscribe(data=>{
   
  this.isLoading = true;
  this.getOrders();
  this.openSnackBar("data checked") ;
})
