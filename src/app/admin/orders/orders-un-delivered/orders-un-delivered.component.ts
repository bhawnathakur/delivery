...
@Component({
  selector: 'app-orders-un-delivered',
  templateUrl: './orders-un-delivered.component.html',
  styleUrls: ['./orders-un-delivered.component.scss']
})
export class OrdersUnDeliveredComponent implements OnInit {
  anElement1 : any;
  orders: Order[] = [];
  url: string;
  drivers: any;
...
setpage: any ;
  ...
  ngOnInit(): void {
    ...
    this.constantsservice.comingFromdUnDelivered=true;
    this.setpage =this.constantsservice.setUnDeliveredPageIndex;
...
getOrders(){
  this.service.getAdminUndelivered().subscribe((data: any)=>{
    this.orders = data;
...
paginationClicked(evt){
  console.log(evt);
  this.constantsservice.setUnDeliveredPageIndex=evt.pageIndex ;
